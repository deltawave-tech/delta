...
from paperqa_temp.docs import Docs
from paperqa_temp.settings import AgentSettings
...
async def run_agent(
    ...
    **runner_kwargs,
    ...
    if isinstance(agent_type, str) and agent_type.lower() == FAKE_AGENT_TYPE:
        ...
        print(f'Will run aviary agent with args: query:{query} docs:{docs} tool_selector_or_none:{tool_selector_or_none} runner_kwargs:{runner_kwargs}')
        session, agent_status = await run_aviary_agent(
            query, docs, tool_selector_or_none, **runner_kwargs
        )
        print(f'ran aviary agent with args: query:{query} docs:{docs} tool_selector_or_none:{tool_selector_or_none} runner_kwargs:{runner_kwargs}')
