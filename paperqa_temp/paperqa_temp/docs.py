...
from paperqa_temp.clients import DEFAULT_CLIENTS, DocMetadataClient
from paperqa_temp.core import llm_parse_json, map_fxn_summary
from paperqa_temp.llms import (
    EmbeddingModel,
    LLMModel,
    NumpyVectorStore,
    PromptRunner,
...
from paperqa_temp.paths import PAPERQA_DIR
from paperqa_temp.prompts import CANNOT_ANSWER_PHRASE
from paperqa_temp.readers import read_doc
from paperqa_temp.settings import MaybeSettings, get_settings
from paperqa_temp.types import (
    Doc,
    DocDetails,
    DocKey,
    Embeddable,
...
from paperqa_temp.utils import (
...
class Docs(BaseModel):
    ...
    async def aquery(  # noqa: PLR0912
        ...
        contexts_old = session.contexts
        # print(f'-'*100)
        # print(f"Contexts Old: {contexts_old}")
        # print(f'-'*100)
        ...
        # print('-'*100)
        # print('Skip answer Config: ',answer_config.skip_answer_generation)
        # print('-'*100)
        bib = {}
        ...
        if answer_config.skip_answer_generation:
            print('PRINT SKIIIIPPPER')
            # Perhaps set any placeholders you need:
            session.answer = context_str
            session.formatted_answer = context_str
            # Keep the references or set them empty:
            session.references = context_str
            # Keep the context if you want to pass it back:
            session.contexts = filtered_contexts
            # session.context = context_str
            session.context = contexts_old
            return session
        else:
            with set_llm_session_ids(session.id):
    ...
    async def aquery_2(  # noqa: PLR0912
        self,
        query: PQASession | str,
        settings: MaybeSettings = None,
        callbacks: list[Callable] | None = None,
        ...
        print(f"Context string: {context_str}")
        with open('summarized_chunks.txt', 'w') as f:
            f.write(context_str)
